    ...
    def plan(self, start_state, dest_state):
        ...
        while Q:
            ...
            for ns in self.get_neighboring_states(s):
                ...
                # Use Manhattan distance as a heuristic for the A*

                g = lambda node: abs(node.x - start_state.x) + abs(node.y - start_state.y)
                h = lambda node: abs(node.x - dest_state.x) + abs(node.y - dest_state.y)
...
if __name__ == "__main__":
    ...
    dest_state = State(595, 140)

    print("astar_planner.py is attempting to find a path from (" + str(start_state.x) + ',' + str(
        start_state.y) + ') to (' + str(dest_state.x) + ',' + str(dest_state.y) + ') in the image named ' + sys.argv[
              1] + '.')
