...
class HuskyMapper:
    ...
    def from_laser_to_map_coordinates(self, points_in_baselaser_frame):
        ...
        rotation_matrix = [
            [np.cos(self.yaw_map_baselaser), -np.sin(self.yaw_map_baselaser), 0],
            [np.sin(self.yaw_map_baselaser), np.cos(self.yaw_map_baselaser), 0],
            [0, 0, 1]
        ]

        return points_in_baselaser_frame + np.matmul(rotation_matrix,
                                                     [self.baselaser_x_in_map, self.baselaser_y_in_map, 0])
        # return points_in_baselaser_frame + [self.baselaser_x_in_map * cos(self.yaw_map_baselaser),
        #                                   self.baselaser_y_in_map * sin(self.yaw_map_baselaser), 0]
    ...
    def is_in_field_of_view(self, robot_row, robot_col, laser_theta, row, col):
        ...
        distanceVectorMagnitude = np.sqrt(((row - robot_row) ** 2) + (col - robot_col) ** 2) * self.ogm.meters_per_cell
        distanceVectorTheta = math.atan2(row - robot_row, col - robot_col)

        differenceAngle = atan2(sin(distanceVectorTheta - laser_theta), cos(distanceVectorTheta - laser_theta))

        if self.min_laser_range < distanceVectorMagnitude < self.max_laser_range and self.min_laser_angle < differenceAngle < self.max_laser_angle:
            return True
    ...
    def inverse_measurement_model(self, row, col, robot_row, robot_col, robot_theta_in_map, beam_ranges, beam_angles):
        ...
        #
        r = np.sqrt(((row - robot_row) ** 2) + (col - robot_col) ** 2) * self.ogm.meters_per_cell  # MUST CHANGE THIS
        diff_angle = math.atan2(row - robot_row, col - robot_col) - robot_theta_in_map  # MUST CHANGE THIS
    ...
    def process_laser_scan(self, ranges_in_baselaser_frame):
        ...
        #ranges_in_baselaser_frame = [i for i in ranges_in_baselaser_frame if i != math.inf]

        #deal with inf values in data
        ranges = []
        for point in ranges_in_baselaser_frame:
            ranges.append(min(point, self.max_laser_angle))

        radiusAndThetaData = zip(self.angles_in_baselaser_frame, ranges)
        points_xyz_in_baselaser_frame = np.array(
            [[r * cos(angle), r * sin(angle), 0] for angle, r in radiusAndThetaData])  # CHANGE THIS
